...
- **Per-screen config** — Screens settings gain CAP fields: state, county FIPS code(s), severity floor, on/off toggle. Pull defaults from screen timezone / IP geolocation when first paired. **Manual config shipped; state selector, conservative state suggestions, and FIPS normalization shipped; IP geolocation remains.**
...
- **Bilingual display controls** — per-screen CAP settings can choose English, Spanish labels, or English + Spanish labels, with an optional Spanish safety note shown below the official alert text. Official NWS alert text is preserved as received. **Shipped 2026-05-10.**
