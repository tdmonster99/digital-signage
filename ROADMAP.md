...
**Status:** 5.4.0 browser player diagnostics shipped 2026-05-03. 5.4.1 Android initial WebView shell landed 2026-05-03; it still needs a real device/emulator build/install smoke test, signing path, and kiosk provisioning checklist before calling Android production-ready. BrightSign is deferred until a customer/RFP requires it.
...
- **5.4.1 Android TV / Android signage APK** — initial native WebView shell added under `player-android/`: hosted player URL, auto-launch receiver, fullscreen/keep-awake behavior, network and renderer recovery, reset/reload maintenance menu, Android shell diagnostics, and sideload build notes. Remaining before production-ready: build/install on Android TV/signage hardware, sign release APK/AAB, document kiosk/device-owner provisioning, and validate offline/media behavior.
